...
```
For the joined timeseries with metrics, the observational unit is one timeseries that is available in the relational database, stored with the assessment its from (which we can use to join with assessments), the stock its about, the years it covers; and after joining with metrics, information about what type of time series it is; aka what quantity it is a time series of and what units the measurements are in. This allows all the metadata for all available time series to be in one table.
...
The 5 most commonly reported metrics in this time frame are CdivMEANC-ratio, TCbest-MT, TL-MT, SSB-MT, and R-E00. Respectively, these represent the catch/mean catch (normalized catch); general total catch (total catch if available, total landings if not available, guaranteed to use metric tons); total landings (I believe this means the number of caught and non released cod), spawning stock biomass (according to the international seafood sustainability foundation (ISSF), the total weight of all sexually mature female cod), and recruits (according to the ISSF, the number of fish in the youngest age group that is still considered exploitable by fisheries).
